...
def callback():
    try:
        ...
        # Toolforge proxy wymusza poprawne parsowanie request.url
...
def handle_upload():
    ...
    try:
        downloaded_file, safe_title, description, comment = download_media(
            yt_url, media_type, timestamp, user_proxy=user_proxy
        )
...
def local_create_job():
    ...
    with _db() as conn:
        conn.execute(
            """
            INSERT INTO jobs
                (id, browser_token, url, media_type, timestamp, status, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, 'waiting', ?, ?)
            """,
            (job_id, browser_token, url, media_type, None if timestamp is None else str(timestamp), now, now)
        )
